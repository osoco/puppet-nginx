...
#
#   [*rewrite*]          - Specifies a rewrite expression for the server config.
#   [*access_log*]       - Specifies the access log file path. If it is not specified will be used the server name by default.
#   [*default_location*] - Enables or disables the default location generation for this server. You can disable to specify one or more
#                          locations using nginx::resource::location.
#
# Actions:
#
# Requires:
...
define nginx::resource::vhost(
	...
	file { "${nginx::config::nx_temp_dir}/nginx.d/${name}-001":
		...
		ginx::resource::location {"${name}-default":
		        ensure	     => $ensure,
		 	vhost        => $name,
			ssl          => $ssl,
			location     => '/', 	
